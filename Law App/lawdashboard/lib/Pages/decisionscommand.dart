...
getdata() async {
  Data.listData.clear();
  Data.listData = await Data.getData("DecisionsCommandControll/GetAll");
  Data.clear();
  Get.forceAppUpdate();
}

class DecisionsCommandControll extends StatefulWidget {
  final String title;
  const DecisionsCommandControll({required this.title, super.key});

  @override
  State<DecisionsCommandControll> createState() =>
      _DecisionsCommandControllState();
}

class _DecisionsCommandControllState extends State<DecisionsCommandControll> {
  @override
  void initState() {
    getdata();
    super.initState();
    ...
    return Directionality(
      ...
      child: Scaffold(
        floatingActionButton: GestureDetector(
          onTap: () => Get.to(
              () => const ADDDecisionsCommandControll(
                    id: '0',
                  ),
...
class ViewDecisionsCommandControll extends StatelessWidget {
  final Map item;
  final String title;
  const ViewDecisionsCommandControll(
                    ...
                    children: [
                      CustomButton(
                          ...
                          ontap: () {
                            ...
                            Get.to(() => ADDDecisionsCommandControll(
                                id: "${item['id']}"));
                          }),
                      CustomButton(
                          data: "44".tr,
                          ontap: () {
                            Data.deleteData(
                                "DecisionsCommandControll/Delete?id=${item['id']}");
                            getdata();
                          }),
                    ],
                  ),
...
class ADDDecisionsCommandControll extends StatelessWidget {
  final String id;
  const ADDDecisionsCommandControll({required this.id, super.key});
                ...
                Center(
                  child: Container(
                    ...
                    child: CustomButton(
                      ...
                      ontap: () {
                        if (id == "0") {
                          Data.insertData("DecisionsCommandControll/Insert", {
                            "id": id,
                            "dscrp": Data.details.text,
                          });
                          getdata();
                        } else {
                          Data.updateData("DecisionsCommandControll/Update", {
                            "id": id,
                            "dscrp": Data.details.text,
                          });
                          getdata();
