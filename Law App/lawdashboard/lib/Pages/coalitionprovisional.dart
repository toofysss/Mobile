...
getdata() async {
  Data.listData.clear();
  Data.listData = await Data.getData("CoalitionProvisional/GetAll");
  Data.clear();

  Get.forceAppUpdate();
}

class CoalitionProvisional extends StatefulWidget {
  final String title;
  const CoalitionProvisional({required this.title, super.key});

  @override
  State<CoalitionProvisional> createState() => _CoalitionProvisionalState();
}

class _CoalitionProvisionalState extends State<CoalitionProvisional> {
  @override
  void initState() {
    getdata();
    super.initState();
...
class ViewCoalitionProvisional extends StatelessWidget {
  final Map item;
  final String title;
  const ViewCoalitionProvisional(
                    ...
                    children: [
                      CustomButton(
                          ...
                          ontap: () {
                            ...
                            Get.to(() =>
                                ADDCoalitionProvisional(id: "${item['id']}"));
                          }),
                      CustomButton(
                          data: "44".tr,
                          ontap: () {
                            Data.deleteData(
                                "CoalitionProvisional/Delete?id=${item['id']}");
                            getdata();
                          }),
                    ],
                  ),
...
class ADDCoalitionProvisional extends StatelessWidget {
  final String id;
  const ADDCoalitionProvisional({required this.id, super.key});
                ...
                Center(
                  child: Container(
                    ...
                    child: CustomButton(
                      ...
                      ontap: () {
                        if (id == "0") {
                          Data.insertData("CoalitionProvisional/Insert", {
                            "id": id,
                            "dscrp": Data.details.text,
                          });
                          getdata();
                        } else {
                          Data.updateData("CoalitionProvisional/Update", {
                            "id": id,
                            "dscrp": Data.details.text,
                          });
                          getdata();
