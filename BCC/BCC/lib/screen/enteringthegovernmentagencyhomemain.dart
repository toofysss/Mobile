...
class EnteringthegovernmentagencyHome extends StatefulWidget {
  const EnteringthegovernmentagencyHome({super.key});

  @override
  State<EnteringthegovernmentagencyHome> createState() =>
      _EnteringthegovernmentagencyHomeState();
}

class _EnteringthegovernmentagencyHomeState
    extends State<EnteringthegovernmentagencyHome> {
// فلترة
  filter() async {
    if (DlailPageClass.fetchedCompanies.isEmpty) {
      ...
      return;
    } else if (DlailPageClass.search.isEmpty) {
      setState(() {
        DlailPageClass.fetchedCompanies.clear();
        DlailPageClass.url = DlailPageClass.name.text;
        DlailPageClass.getdata(context, DlailPageClass.url);
      });
                                ...
                                child: ListTile(
                                  title: DropdownButtonHideUnderline(
                                    child: DropdownButton<String>(
                                      ...
                                      hint: CustomText(
                                          color: All.color1,
                                          data: LoginClass.search,
                                          size: All.textsize),
                                      onChanged: (value) {
                                        setState(() {
                                          LoginClass.search = value!;
                                        });
                                      },
                                      items: [
                                        "227".tr,
                                ...
                                return Padding(
                                  ...
                                  child: ListView.builder(
                                      ...
                                      itemBuilder:
                                        ...
                                        // دليل التاجر
                                        if (LoginClass.search == "227".tr) {
                                          return const CustomLoading(
                                            color: Colors.black,
                                          );
                                        } else if (LoginClass.search ==
                                            "228".tr) {
                                          return const CustomLoading(
                                            color: Colors.black,
                                          );
